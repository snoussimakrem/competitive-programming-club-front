...
function buildItems(pool, seg) {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', alt: '', club: '', description: '' }));
  }

  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', club: '', description: '' };
    }
    return { 
      src: image.src || '', 
      alt: image.alt || '', 
...
export default function DomeGallery({
  ...
  const openItemFromElement = useCallback(
    el => {
      ...
      const club = parent.dataset.club || '';
      const description = parent.dataset.description || '';
      
      const contentWrapper = document.createElement('div');
      contentWrapper.style.cssText = 'position: relative; width: 100%; height: 100%;';
      ...
      if (club || description) {
        ...
        if (club) {
          ...
          clubName.style.cssText = 'font-size: 1.5rem; font-weight: 700; margin-bottom: 0.5rem;';
          infoOverlay.appendChild(clubName);
        }
        
        if (description) {
          const desc = document.createElement('p');
          desc.textContent = description;
          desc.style.cssText = 'font-size: 0.95rem; color: rgba(255,255,255,0.9); line-height: 1.5;';
            ...
            {items.map((it, i) => (
              ...
              >
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={it.alt || it.club || 'Open image'}
                  onClick={onTileClick}
